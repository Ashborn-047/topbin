...
if (!ARCHIVE_FILE.startsWith(ROOT + path.sep)) {
  console.error(`Security Error: Archive path is outside the allowed directory.`);
  process.exit(1);
}
if (!DEST_FILE.startsWith(ROOT + path.sep)) {
  console.error(`Security Error: Destination path is outside the allowed directory.`);
  process.exit(1);
}
