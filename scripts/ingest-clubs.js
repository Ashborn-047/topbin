...
if (!ARCHIVE_FILE.startsWith(ROOT + path.sep)) {
  console.error(`Error: Invalid archive path. Access outside of root is not allowed.`);
  process.exit(1);
}

if (!DEST_FILE.startsWith(ROOT + path.sep)) {
  console.error(`Error: Invalid destination path. Access outside of root is not allowed.`);
  process.exit(1);
}
