...
// Ensure resolved paths stay within ROOT directory to prevent path traversal
if (!ARCHIVE_FILE.startsWith(ROOT + path.sep)) {
  console.error(`Invalid source archive path. Path traversal detected: ${ARCHIVE_FILE}`);
  process.exit(1);
}

if (!DEST_FILE.startsWith(ROOT + path.sep)) {
  console.error(`Invalid destination path. Path traversal detected: ${DEST_FILE}`);
  process.exit(1);
}
